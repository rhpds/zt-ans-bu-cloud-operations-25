...
. **Navigate to the Ansibel Automation Platform UI.**
+
...
+
Select the **Sources** tab and click the **Launch Inventory** button (the rocket ship icon). This will update the host list from AWS.
...
+
Enter the following text to the *Extra Variables* text section.
+
...
+
Scroll to the bottom, click **Create job template**, and then **Launch template**. Observe the output of the job run.
...
+
Hover over the `AWS Source` node you just created, click the **ellipsis** icon, and select *Add Step and link*. Configure it as follows:
+
--
a. **Run type:** Ensure `Run On Success` is selected.
...
+
In the top left corner of the Visualizer, click **Save**. Then, navigate back to the **Templates** page and launch the `WORKFLOW - Retrieve EC2 information` template.
